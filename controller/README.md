...
### Current semantic-recovery contract

- The Git-tracked default board and a separately installed local TEST board are different artifacts. Every regenerated board requires its own source/runtime digest and reload verification; prior version text or an old mock does not attest it. Boards missing required source-expiration metadata now lock and must be rebuilt, not retimestamped.
- Projection families must match the player's scoring kind. Unmatched/ambiguous fresh ADP imports clear old values and retain the labeled rank fallback. Next-pick survival conditions on current availability and opposing pick opportunities; consecutive owned picks have probability 1 for an unchosen player. These probabilities are not a new calibration claim.
- TEST replacement values must match the observed team count and starter slots. Runtime source expiry follows every required source clock, not only board generation. Automatic and manual-only coverage are separate counts.
- Use Yahoo Autodraft OFF and an EMPTY Yahoo queue. The extension's exact-ID ladder is not Yahoo's queue. Historical queue-first instructions do not apply to this runner or authorize REAL execution.
- Between decisions only, unreadable Autodraft/queue or split title/banner signals pause observation for at most 250ms with receipts and no Yahoo actions. Persistent uncertainty, actual Autodraft activity, nonempty/ambiguous queue contents, identity or lease loss fail closed. During a pending decision or controller execution, safety remains immediate and strict. There is no retry after failure or ambiguous click.
- Keep the draft tab foreground, arm only after verified prestart readiness, and never reload or improvise recovery in the live client. A rescue forfeits clean acceptance. Export untouched receipts and exact final roster; public mock proof cannot substitute for 19-round IDP qualification. REAL 420010 stays disabled.
...
- TEST `542830`, Yahoo team `3`, requires its own league/model/schema identity. Its complete enabled scoring rows were observed on 2026-09-05 and fingerprinted in the runner contract. The Git-tracked board is REAL `420010` data, not relabeled TEST data: TEST ARM requires a separately built, deployed, fresh TEST board. Public mocks rehearse execution, not TEST scoring calibration. Real execution remains disabled.
...
The runner waits for the controller's exact owned-turn signal before constructing a clickable ladder; TEST view discovery runs off-turn as described above. Each decision uses one freshly verified visible pool. It computes exact marginal Weeks 1–17 utility over the best legal weekly lineups, so byes, explicit missed games, and QB2 substitution value are priced directly. Once an offensive player no longer adds a starter slot, the engine retains a bounded, position/depth-dependent share of positive VOR (RB 30%/12%, WR 24%/12%, QB 6%/1%, TE 5%/1%) as bench opportunity value instead of treating useful depth as zero. It then estimates continuous next-turn survival from the held-out league curve (or a visibly uncalibrated market fallback) and evaluates the best one-turn alternatives. Explicitly supplied position pressure alters only survival, never projection, replacement level, or marginal utility; table disappearance supplies no pressure.
...
The repository root is also a dependency-free Manifest V3 extension. Load the repository directory as an unpacked extension in Chrome. It requests no general extension permissions and runs MOCK/TEST execution only on Yahoo's public mock waiting room and exact League Two surfaces. Version `0.16.4` adds read-only advice on exact REAL league 420010 / team 7 surfaces. It loads pure decision helpers but never creates a runner, loads the click controller, or accepts mutation commands. **Reload & Verify** is available only on idle, off-clock TEST/MOCK setup surfaces and is disabled inside the live draft client. It reloads the unpacked extension, refreshes the same Yahoo tab, and requires a new extension-session boot ID plus a healthy manifest-version/runtime-digest handshake before restoring controls. A missing rail after reload means Chrome could not inject the extension and requires inspection. Stale, missing, or mismatched attestations remain visibly locked.

REAL advice uses a separately exported `extension/yahoo-real-board.js`; the tracked file is an empty, fail-closed placeholder. The TEST board must never substitute for it. An approved deployment exports with `node analysis/export-extension-board.mjs --input=<verified-player-board.json> --output=<staging>/yahoo-real-board.js --mode=REAL`, verifies every required source expiry, installs the artifact, and attests/reloads while DISARMED. Full refresh output also includes `yahoo-real-board-v15.js` for this purpose. Recheck freshness close enough to draft time to cover the whole expected draft plus margin; regeneration never refreshes source observations. No mid-draft deployment.

The REAL rail validates all observed scoring-table rows, league/model/schema, and exact replacement roster. In the draft client it requires the rendered 12-team snake strip, exact owned roster IDs, consistent turn signals and fresh enabled Draft rows. Advice is explicitly **VISIBLE POOL ONLY**: hidden players are not called available, unmodelled rows are counted and omitted, and unavailable/ambiguous inputs withhold recommendations. Off-turn disabled rows are not positive availability evidence. Roster membership is observed; displayed names do not certify Yahoo slot assignment. REAL controls, pins, hotkeys and clicks remain disabled. No live LLM committee or manager prediction is applied.

Reduced projected games for CLEAR-health players cap weekly availability without automatically excluding them; true injury/manual-only and independent-source gates remain. Refresh health lists every top-100 Yahoo offensive exclusion with its reason. Conditional lookahead evaluates all legal pairs after the proposed first pick and retains the full survival chain. Rehearsal opponents use manual-eligible identities independently of our automatic policy and their own snake-seat roster constraints. Position caps are modelling assumptions, not verified Yahoo limits. These results establish offline feasibility and compute latency—not clean live drafting or predictive team quality.
...
- `opponent-window.mjs` and `opponent-war-room.mjs` produce offline descriptive
  manager cards and snake windows. They are not connected to live ranking.
  The deployed runner uses market timing with zero live run-pressure input;
  rehearsals use the same setting. Manager cards never change live BPA.
...
- `draft-committee.mjs` creates compact, packet-hashed candidate ballots and
  ...
  the deadline. This is an offline receipt-review tool, not a live model call
  or decision committee in the extension.
