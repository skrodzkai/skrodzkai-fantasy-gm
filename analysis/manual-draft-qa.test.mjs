...
test('shipped UI favorites, grey rows, hide, reload, tabs and order handlers work together',async()=>{
 ...
 const fields=ui.elements.get('orderFields').querySelectorAll('select');fields[0].value='6';fields[1].value='5';ui.elements.get('saveOrder').onclick();assert.equal(ui.elements.get('strategy').children.filter(e=>e.tagName==='ARTICLE').length,19);
