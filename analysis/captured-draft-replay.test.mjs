...
test("fails closed unless every captured-session contract is present", () => {
  ...
  const required = "generic `D` selection must precede every LB/CB/S selection owner intervened at the final moment No clean automated-draft claim is valid";
  ...
  assert.equal(replayCapturedDraft({ uiMap: {}, postmortem: "" }).status, "FAIL");
});
