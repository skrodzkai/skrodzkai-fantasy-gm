(function(){
  ...
  async function observe(){
    ...
    try{
      ...
      const observation=api.readResults(doc,{leagueId});
      for(const p of observation.picks)if(!p.yahooId){
        const matches=board.players.filter(x=>x.position==='DEF'&&x.name===p.name);
        if(matches.length!==1)throw Error('Defense identity unresolved');p.yahooId=matches[0].yahooId;
      }
  ...
  timer=setInterval(observe,2000);void observe();
})();
